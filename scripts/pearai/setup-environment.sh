...
app_dir=$(pwd)
target_path="$app_dir/extensions/lycheeai-submodule/extensions/vscode"
link_path="$app_dir/extensions/lycheeai-ref"

# Run the base functionality
echo -e "\nInitializing sub-modules..."

# Check if the submodule directory already exists
if [ -d "$app_dir/extensions/lycheeai-submodule" ]; then
    echo "Removing existing lycheeai-submodule directory"
    execute "rm -rf $app_dir/extensions/lycheeai-submodule" "Failed to remove existing lycheeai-submodule directory"
fi
...
execute "cd ./extensions/lycheeai-submodule" "Failed to change directory to extensions/lycheeai-submodule"
