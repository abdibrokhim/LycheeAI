...
export class KeybindingResolver {
	...
	private _getKeybindingPriority(item: ResolvedKeybindingItem): number {
		...
		if (item.extensionId === 'lycheeai.lycheeai') {
			// todo: remove id hardcode and extract extension id from env or settings
			return 2; // lycheeai AI keybindings
		}
	...
	public static handleRemovals(rules: ResolvedKeybindingItem[]): ResolvedKeybindingItem[] {
    	...
    	return result.sort((a, b) => {
			const priorityA = a.isDefault ? (a.extensionId === 'lycheeai.lycheeai' ? 2 : 1) : 3;
			const priorityB = b.isDefault ? (b.extensionId === 'lycheeai.lycheeai' ? 2 : 1) : 3;
