...
export class ExtHostNotebookController implements ExtHostNotebookShape {
	...
	private static _registerApiCommands(extHostCommands: ExtHostCommands) {
		...
		const notebookTylycheeaig = ApiCommandArgument.String.with('notebookType', 'A notebook type');

		const commandDataToNotebook = new ApiCommand(
			'vscode.executeDataToNotebook', '_executeDataToNotebook', 'Invoke notebook serializer',
			[notebookTylycheeaig, new ApiCommandArgument<Uint8Array, VSBuffer>('data', 'Bytes to convert to data', v => v instanceof Uint8Array, v => VSBuffer.wrap(v))],
		...
		const commandNotebookToData = new ApiCommand(
			...
			[notebookTylycheeaig, new ApiCommandArgument<vscode.NotebookData, SerializableObjectWithBuffers<NotebookDataDto>>('NotebookData', 'Notebook data to convert to bytes', v => true, v => new SerializableObjectWithBuffers(typeConverters.NotebookData.from(v)))],
